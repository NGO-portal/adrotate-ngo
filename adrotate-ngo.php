...
/*
Original Version: 3.15.5.1
To update, copy the files in AdRotate-NGO from the updated AdRotate, and then search and replace all instances of $wpdb->prefix with $wpdb->base_prefix
...
If you enable this plugin at the same site as AdRotate your site will come tumbling down on you... Since this is a stripped copy of AdRotate the functions are named the same as in AdRotate... Be careful. If it happens, just rename this plugin to something else using ftp or ssh, and it will be deactivated by Wordpress. All the credits go to Arnan.
*/
...
/*--- AdRotate values ---------------------------------------*/
define("ADROTATE_DISPLAY", '3.15.5.1');
...
function adrotate_manage() {
	...
	<div class="wrap">
		...
		$allbanners = $wpdb->get_results("SELECT `id`, `title`, `type`, `tracker`, `weight` FROM `{$wpdb->base_prefix}adrotate` WHERE (`type` != 'empty' OR `type` != 'a_empty' OR `type` != 'queue') ORDER BY `sortorder` ASC, `id` ASC;");

		$activebanners = $errorbanners = $disabledbanners = false;
		foreach($allbanners as $singlebanner) {
			$starttime = $stoptime = 0;
			$starttime = $wpdb->get_var("SELECT `starttime` FROM `{$wpdb->base_prefix}adrotate_schedule`, `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` = '".$singlebanner->id."' AND `schedule` = `{$wpdb->base_prefix}adrotate_schedule`.`id` ORDER BY `starttime` ASC LIMIT 1;");
			$stoptime = $wpdb->get_var("SELECT `stoptime` FROM `{$wpdb->base_prefix}adrotate_schedule`, `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` = '".$singlebanner->id."' AND `schedule` = `{$wpdb->base_prefix}adrotate_schedule`.`id` ORDER BY `stoptime` DESC LIMIT 1;");
...
function adrotate_manage_schedules() {
	...
	<div class="wrap">
		...
		<table class="widefat" style="margin-top: .5em">
			...
			<tbody>
		<?php
		$schedules = $wpdb->get_results("SELECT * FROM `{$wpdb->base_prefix}adrotate_schedule` WHERE `name` != '' ORDER BY `id` ASC;");
		if($schedules) {
			$class = '';
			foreach($schedules as $schedule) {
				$schedulesmeta = $wpdb->get_results("SELECT `ad` FROM `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `group` = 0 AND `user` = 0 AND `schedule` = ".$schedule->id.";");
