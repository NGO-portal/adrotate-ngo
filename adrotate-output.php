...
function adrotate_ad($banner_id, $individual = true, $group = null, $site = 0) {
	...
	if($banner_id) {			
		$banner = $wpdb->get_row($wpdb->prepare("SELECT `id`, `title`, `bannercode`, `tracker`, `image`, `responsive` FROM `{$wpdb->base_prefix}adrotate` WHERE `id` = %d AND (`type` = 'active' OR `type` = '2days' OR `type` = '7days');", $banner_id));
...
function adrotate_group($group_ids, $fallback = 0, $weight = 0, $site = 0) { 
	...
	if($group_ids) {
		...
		foreach($group_array as $key => $value) {
			$group_select .= " `{$wpdb->base_prefix}adrotate_linkmeta`.`group` = {$value} OR";
		}
		$group_select = rtrim($group_select, " OR");

		$group = $wpdb->get_row($wpdb->prepare("SELECT * FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `name` != '' AND `id` = %d;", $group_array[0]));
		...
		if($group) {
			...
			$ads = $wpdb->get_results(
				"SELECT 
					`{$wpdb->base_prefix}adrotate`.`id`, 
					`{$wpdb->base_prefix}adrotate`.`title`, 
					`{$wpdb->base_prefix}adrotate`.`bannercode`, 
					`{$wpdb->base_prefix}adrotate`.`image`, 
					`{$wpdb->base_prefix}adrotate`.`responsive`, 
					`{$wpdb->base_prefix}adrotate`.`tracker`, 
					`{$wpdb->base_prefix}adrotate_linkmeta`.`group`
				FROM 
					`{$wpdb->base_prefix}adrotate`, 
					`{$wpdb->base_prefix}adrotate_linkmeta` 
				WHERE 
					({$group_select}) 
					AND `{$wpdb->base_prefix}adrotate_linkmeta`.`user` = 0 
					AND `{$wpdb->base_prefix}adrotate`.`id` = `{$wpdb->base_prefix}adrotate_linkmeta`.`ad` 
					AND (`{$wpdb->base_prefix}adrotate`.`type` = 'active' 
						OR `{$wpdb->base_prefix}adrotate`.`type` = '2days'
						OR `{$wpdb->base_prefix}adrotate`.`type` = '7days')
				GROUP BY `{$wpdb->base_prefix}adrotate`.`id`
				ORDER BY `{$wpdb->base_prefix}adrotate`.`id`;");
...
function adrotate_inject_posts($post_content) { 
	...
	if(is_page()) {
		...
		$ids = $wpdb->get_results("SELECT `id`, `page`, `page_loc`, `page_par` FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `page_loc` > 0 AND  `page_loc` < 5;");
	...
	if(is_single()) {
		...
		$ids = $wpdb->get_results("SELECT `id`, `cat`, `cat_loc`, `cat_par` FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `cat_loc` > 0 AND `cat_loc` < 5;");
...
function adrotate_preview($banner_id) {
	...
	if($banner_id) {
		...
		$banner = $wpdb->get_row($wpdb->prepare("SELECT * FROM `{$wpdb->base_prefix}adrotate` WHERE `id` = %d;", $banner_id));
...
function adrotate_custom_javascript() {
	...
	$groups = $wpdb->get_results("SELECT `id`, `adspeed` FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `name` != '' AND `modus` = 1 ORDER BY `id` ASC;");
	if($groups) {
		$output = "<!-- AdRotate JS -->\n";
...
function adrotate_custom_css() {
	...
	$groups = $wpdb->get_results("SELECT `id`, `modus`, `gridrows`, `gridcolumns`, `adwidth`, `adheight`, `admargin`, `align` FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `name` != '' ORDER BY `id` ASC;");
	if($groups) {
