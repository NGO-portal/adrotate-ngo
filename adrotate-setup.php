...
function adrotate_uninstall_setup() {
	...
	// Drop MySQL Tables
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_groups`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_tracker`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_linkmeta`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_stats`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_stats_archive`");
	$wpdb->query("DROP TABLE IF EXISTS `{$wpdb->base_prefix}adrotate_schedule`");
...
function adrotate_dummy_data() {
	...
	$no_ads = $wpdb->get_var("SELECT `id` FROM `{$wpdb->base_prefix}adrotate` LIMIT 1;");
	$no_schedules = $wpdb->get_var("SELECT `id` FROM `{$wpdb->base_prefix}adrotate_schedule` LIMIT 1;");
	$no_linkmeta = $wpdb->get_var("SELECT `id` FROM `{$wpdb->base_prefix}adrotate_linkmeta` LIMIT 1;");

	if(is_null($no_ads) AND is_null($no_schedules) AND is_null($no_linkmeta)) {
		// Demo ad 1
	    $wpdb->insert("{$wpdb->base_prefix}adrotate", array('title' => 'Demo ad 468x60', 'bannercode' => '&lt;a href=\&quot;http:\/\/www.adrotateforwordpress.com\&quot;&gt;&lt;img src=\&quot;http://ajdg.solutions/assets/dummy-banners/adrotate-468x60.jpg\&quot; /&gt;&lt;/a&gt;', 'thetime' => $now, 'updated' => $now, 'author' => $current_user->user_login, 'imagetype' => '', 'image' => '', 'tracker' => 'N', 'desktop' => 'Y', 'mobile' => 'Y', 'tablet' => 'Y', 'responsive' => 'N', 'type' => 'active', 'weight' => 6, 'sortorder' => 0, 'budget' => 0, 'crate' => 0, 'irate' => 0, 'cities' => serialize(array()), 'countries' => serialize(array())));
	    $ad_id = $wpdb->insert_id;
		$wpdb->insert("{$wpdb->base_prefix}adrotate_schedule", array('name' => 'Schedule for ad '.$ad_id, 'starttime' => $now, 'stoptime' => $in84days, 'maxclicks' => 0, 'maximpressions' => 0, 'spread' => 'N', 'hourimpressions' => 0, 'daystarttime' => '0000', 'daystoptime' => '0000', 'day_mon' => 'Y', 'day_tue' => 'Y', 'day_wed' => 'Y', 'day_thu' => 'Y', 'day_fri' => 'Y', 'day_sat' => 'Y', 'day_sun' => 'Y'));
	    $schedule_id = $wpdb->insert_id;
		$wpdb->insert("{$wpdb->base_prefix}adrotate_linkmeta", array('ad' => $ad_id, 'group' => 0, 'user' => 0, 'schedule' => $schedule_id));
		unset($ad_id, $schedule_id);
	
		// Demo ad 2
	    $wpdb->insert("{$wpdb->base_prefix}adrotate", array('title' => 'Demo ad 200x200', 'bannercode' => '&lt;a href=\&quot;http:\/\/www.adrotateforwordpress.com\&quot;&gt;&lt;img src=\&quot;http://ajdg.solutions/assets/dummy-banners/adrotate-200x200.jpg\&quot; /&gt;&lt;/a&gt;', 'thetime' => $now, 'updated' => $now, 'author' => $current_user->user_login, 'imagetype' => '', 'image' => '', 'tracker' => 'N', 'desktop' => 'Y', 'mobile' => 'Y', 'tablet' => 'Y', 'responsive' => 'N', 'type' => 'active', 'weight' => 6, 'sortorder' => 0, 'budget' => 0, 'crate' => 0, 'irate' => 0, 'cities' => serialize(array()), 'countries' => serialize(array())));
	    $ad_id = $wpdb->insert_id;
		$wpdb->insert("{$wpdb->base_prefix}adrotate_schedule", array('name' => 'Schedule for ad '.$ad_id, 'starttime' => $now, 'stoptime' => $in84days, 'maxclicks' => 0, 'maximpressions' => 0, 'spread' => 'N', 'hourimpressions' => 0, 'daystarttime' => '0000', 'daystoptime' => '0000', 'day_mon' => 'Y', 'day_tue' => 'Y', 'day_wed' => 'Y', 'day_thu' => 'Y', 'day_fri' => 'Y', 'day_sat' => 'Y', 'day_sun' => 'Y'));
	    $schedule_id = $wpdb->insert_id;
		$wpdb->insert("{$wpdb->base_prefix}adrotate_linkmeta", array('ad' => $ad_id, 'group' => 0, 'user' => 0, 'schedule' => $schedule_id));
...
function adrotate_database_install() {
	...
	$found_engine = $wpdb->get_var("SELECT ENGINE FROM `information_schema`.`TABLES` WHERE `TABLE_SCHEMA` = '".DB_NAME."' AND `TABLE_NAME` = '{$wpdb->base_prefix}posts';");
	if(strtolower($found_engine) == 'innodb') {
		$engine = ' ENGINE=InnoDB';
	}

	$found_tables = $wpdb->get_col("SHOW TABLES LIKE '{$wpdb->base_prefix}adrotate%';");

	if(!in_array("{$wpdb->base_prefix}adrotate", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate` (
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_groups", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_groups` (
			`id` mediumint(8) unsigned NOT NULL auto_increment,
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_linkmeta", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_linkmeta` (
			`id` mediumint(8) unsigned NOT NULL auto_increment,
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_schedule", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_schedule` (
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_stats", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_stats` (
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_stats_archive", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_stats_archive` (
			`id` bigint(9) unsigned NOT NULL auto_increment,
			`ad` int(5) unsigned NOT NULL default '0',
			`group` int(5) unsigned NOT NULL default '0',
			`thetime` int(15) unsigned NOT NULL default '0',
	...
	if(!in_array("{$wpdb->base_prefix}adrotate_tracker", $found_tables)) {
		dbDelta("CREATE TABLE `{$wpdb->base_prefix}adrotate_tracker` (
			`id` bigint(9) unsigned NOT NULL auto_increment,
...
function adrotate_database_upgrade() {
	...
	if($adrotate_db_version['current'] < 24) {
		if($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->base_prefix}adrotate_stats_tracker'")) {
			$wpdb->query("RENAME TABLE `{$wpdb->base_prefix}adrotate_stats_tracker` TO `{$wpdb->base_prefix}adrotate_stats`;");
		}
	}
	...
	if($adrotate_db_version['current'] < 25) {
		$wpdb->query("CREATE INDEX `timer` ON `{$wpdb->base_prefix}adrotate_tracker` (timer);");
		$wpdb->query("CREATE INDEX `ipaddress` ON `{$wpdb->base_prefix}adrotate_tracker` (ipaddress);");
		$wpdb->query("CREATE INDEX `ad` ON `{$wpdb->base_prefix}adrotate_stats` (ad);");
		$wpdb->query("CREATE INDEX `thetime` ON `{$wpdb->base_prefix}adrotate_stats` (thetime);");
	}

	// Database: 	26
	// AdRotate:	3.8.1
	if($adrotate_db_version['current'] < 26) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'budget', 'double NOT NULL default \'0\' AFTER `sortorder`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'crate', 'double NOT NULL default \'0\' AFTER `budget`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'irate', 'double NOT NULL default \'0\' AFTER `crate`');
	}

	// Database: 	30
	// AdRotate:	3.8.3.4
	if($adrotate_db_version['current'] < 30) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'wrapper_before', 'longtext NOT NULL AFTER `page_loc`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'wrapper_after', 'longtext NOT NULL AFTER `wrapper_before`');
	}

	// Database: 	32
	// AdRotate:	3.8.4.4
	if($adrotate_db_version['current'] < 32) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'cities', 'text NOT NULL AFTER `irate`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'countries', 'text NOT NULL AFTER `cities`');
		$geo_array = serialize(array());
		$wpdb->query("UPDATE `{$wpdb->base_prefix}adrotate` SET `cities` = '$geo_array' WHERE `cities` = '';");
		$wpdb->query("UPDATE `{$wpdb->base_prefix}adrotate` SET `countries` = '$geo_array' WHERE `countries` = '';");
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'geo', 'tinyint(1) NOT NULL default \'0\' AFTER `page_loc`');
	}

	// Database: 	33
	// AdRotate:	3.8.6
	if($adrotate_db_version['current'] < 33) {
		adrotate_del_column("{$wpdb->base_prefix}adrotate_groups", 'token');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'modus', 'tinyint(1) NOT NULL default \'0\' AFTER `name`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'gridrows', 'int(3) NOT NULL default \'2\' AFTER `wrapper_after`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'gridcolumns', 'int(3) NOT NULL default \'2\' AFTER `gridrows`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'admargin', 'int(3) NOT NULL default \'1\' AFTER `gridcolumns`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'adwidth', 'varchar(4) NOT NULL default \'125\' AFTER `admargin`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'adheight', 'varchar(4) NOT NULL default \'125\' AFTER `adwidth`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'adspeed', 'int(5) NOT NULL default \'6000\' AFTER `adheight`');
	}

	// Database: 	36
	// AdRotate:	3.8.10
	if($adrotate_db_version['current'] < 36) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'admargin_bottom', 'int(3) NOT NULL default \'1\' AFTER `admargin`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'admargin_left', 'int(3) NOT NULL default \'1\' AFTER `admargin_bottom`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'admargin_right', 'int(3) NOT NULL default \'1\' AFTER `admargin_left`');
	}

	// Database: 	38
	// AdRotate:	3.9
	if($adrotate_db_version['current'] < 38) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_linkmeta", 'schedule', 'int(5) NOT NULL default \'0\' AFTER `user`');
		$schedules = $wpdb->get_results("SELECT `id`, `ad` FROM {$wpdb->base_prefix}adrotate_schedule ORDER BY `id` ASC;");
		foreach($schedules as $schedule) {
			$wpdb->insert("{$wpdb->base_prefix}adrotate_linkmeta", array('ad' => $schedule->ad, 'group' => 0, 'user' => 0, 'schedule' => $schedule->id), array('%d', '%d', '%d', '%d', '%d'));
			unset($schedule);
		}
		unset($schedules);
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'name', 'varchar(255) NOT NULL default \'\' AFTER `id`');
		adrotate_del_column("{$wpdb->base_prefix}adrotate_schedule", 'ad');

		$schedules = $wpdb->get_results("SELECT `id` FROM {$wpdb->base_prefix}adrotate_schedule WHERE `name` = '' ORDER BY `id` ASC;");
		foreach($schedules as $schedule) {
			$wpdb->update("{$wpdb->base_prefix}adrotate_schedule", array('name' => 'Schedule '.$schedule->id), array('id' => $schedule->id));
			unset($schedule);
		}
		unset($schedules);
	}
	...
	if($adrotate_db_version['current'] < 39) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_tracker", 'country', 'text NOT NULL AFTER `useragent`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_tracker", 'city', 'text NOT NULL AFTER `country`');
	}

	// Database: 	40
	// AdRotate:	3.9.9
	if($adrotate_db_version['current'] < 40) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'responsive', 'varchar(5) NOT NULL default \'N\' AFTER `tracker`');
	}

	// Database: 	41
	// AdRotate:	3.9.12
	if($adrotate_db_version['current'] < 41) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'page_par', 'tinyint(1) NOT NULL default \'0\' AFTER `page_loc`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'cat_par', 'tinyint(1) NOT NULL default \'0\' AFTER `cat_loc`');
	}

	// Database: 	42
	// AdRotate:	3.10
	if($adrotate_db_version['current'] < 42) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'spread', 'varchar(5) NOT NULL default \'N\' AFTER `maximpressions`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'hourimpressions', 'int(15) NOT NULL default \'0\' AFTER `spread`');
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `page_par` `page_par` tinyint(2) NOT NULL default '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `cat_par` `cat_par` tinyint(2) NOT NULL default '0';");
	}

	// Database: 	43
	// AdRotate:	3.10.7
	if($adrotate_db_version['current'] < 43) {
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `admargin` `admargin` int(2) NOT NULL default '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `admargin_bottom` `admargin_bottom` int(2) NOT NULL default '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `admargin_left` `admargin_left` int(2) NOT NULL default '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_groups` CHANGE `admargin_right` `admargin_right` int(2) NOT NULL default '0';");
	}

	// Database: 	44
	// AdRotate:	3.10.8
	if($adrotate_db_version['current'] < 44) {
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'timeframe');
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'timeframelength');
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'timeframeclicks');
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'timeframeimpressions');
	}

	// Database: 	46
	// AdRotate:	3.10.13
	if($adrotate_db_version['current'] < 46) {
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `hourimpressions` `hourimpressions` int(15) NOT NULL default '0';");
	}

	// Database: 	47
	// AdRotate:	3.10.18
	if($adrotate_db_version['current'] < 47) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'align', 'tinyint(1) NOT NULL default \'0\' AFTER `wrapper_after`');
	}

	// Database: 	48
	// AdRotate:	3.11.2b2
	if($adrotate_db_version['current'] < 48) {
		if($wpdb->get_var("SHOW INDEX FROM `{$wpdb->base_prefix}adrotate_tracker` WHERE Key_name = 'bannerid';") !== null) {
			$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_tracker` DROP KEY `bannerid`;");
		}
		if($wpdb->get_var("SHOW INDEX FROM `{$wpdb->base_prefix}adrotate_schedule` WHERE Key_name = 'stoptime';") !== null) {
			$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` DROP KEY `stoptime`;");
		}
		if($wpdb->get_var("SHOW INDEX FROM `{$wpdb->base_prefix}adrotate_schedule` WHERE Key_name = 'ad';") !== null) {
			$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` DROP KEY `ad`;");
		}
		adrotate_del_column("{$wpdb->base_prefix}adrotate_stats", 'block');
		adrotate_del_column("{$wpdb->base_prefix}adrotate_linkmeta", 'block');
	}

	// Database: 	49
	// AdRotate:	3.11.2b3
	if($adrotate_db_version['current'] < 49) {
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_stats` CHANGE `ad` `ad` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_stats` CHANGE `group` `group` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_stats` CHANGE `thetime` `thetime` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_stats` CHANGE `clicks` `clicks` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_stats` CHANGE `impressions` `impressions` INT(15) UNSIGNED NOT NULL DEFAULT '0';");

		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_tracker` CHANGE `timer` `timer` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_tracker` CHANGE `bannerid` `bannerid` INT(15) UNSIGNED NOT NULL DEFAULT '0';");

		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `starttime` `starttime` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `stoptime` `stoptime` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `maxclicks` `maxclicks` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `maximpressions` `maximpressions` INT(15) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `dayimpressions` `dayimpressions` INT(15) UNSIGNED NOT NULL DEFAULT '0';");

		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_linkmeta` CHANGE `ad` `ad` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_linkmeta` CHANGE `group` `group` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_linkmeta` CHANGE `user` `user` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_linkmeta` CHANGE `schedule` `schedule` INT(5) UNSIGNED NOT NULL DEFAULT '0';");
	}

	// Database: 	50
	// AdRotate:	3.11.3b1
	if($adrotate_db_version['current'] < 50) {
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_tracker` CHANGE `ipaddress` `ipaddress` varchar(15) NOT NULL DEFAULT '';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `cbudget` `budget` double NOT NULL default '0';");
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'ibudget');
	}

	// Database: 	51
	// AdRotate:	3.12.5b2
	if($adrotate_db_version['current'] < 51) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'mobile', 'varchar(5) NOT NULL default \'N\' AFTER `tracker`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_groups", 'mobile', 'tinyint(1) NOT NULL default \'0\' AFTER `page_par`');
	}
	
	// Database: 	52
	// AdRotate:	3.13
	if($adrotate_db_version['current'] < 52) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'tablet', 'varchar(2) NOT NULL default \'N\' AFTER `mobile`');
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `tracker` `tracker` varchar(2) NOT NULL default 'N';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `mobile` `mobile` varchar(2) NOT NULL default 'N';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `responsive` `responsive` varchar(2) NOT NULL default 'N';");
	}

	// Database: 	53
	// AdRotate:	3.13.1
	if($adrotate_db_version['current'] < 53) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'daystarttime', 'char(4) NOT NULL default \'0000\' AFTER `dayimpressions`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'daystoptime', 'char(4) NOT NULL default \'0000\' AFTER `daystarttime`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_mon', 'char(1) NOT NULL default \'Y\' AFTER `daystoptime`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_tue', 'char(1) NOT NULL default \'Y\' AFTER `day_mon`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_wed', 'char(1) NOT NULL default \'Y\' AFTER `day_tue`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_thu', 'char(1) NOT NULL default \'Y\' AFTER `day_wed`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_fri', 'char(1) NOT NULL default \'Y\' AFTER `day_thu`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_sat', 'char(1) NOT NULL default \'Y\' AFTER `day_fri`');
		adrotate_add_column("{$wpdb->base_prefix}adrotate_schedule", 'day_sun', 'char(1) NOT NULL default \'Y\' AFTER `day_sat`');
	}

	// Database: 	54
	// AdRotate:	3.14
	if($adrotate_db_version['current'] < 54) {
		adrotate_add_column("{$wpdb->base_prefix}adrotate", 'desktop', 'varchar(2) NOT NULL default \'Y\' AFTER `tracker`');
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `mobile` `mobile` varchar(2) NOT NULL default 'Y';");
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate` CHANGE `tablet` `tablet` varchar(2) NOT NULL default 'Y';");
	}

	// Database: 	55
	// AdRotate:	3.14.1
	if($adrotate_db_version['current'] < 55) {
		$adverts = $wpdb->get_results("SELECT `id`, `bannercode`, `link` FROM {$wpdb->base_prefix}adrotate WHERE `link` != '' ORDER BY `id` ASC;");
		foreach($adverts as $advert) {
			$bannercode = $advert->bannercode;
			$bannercode = str_replace('%link%', $advert->link, $bannercode);
			$wpdb->update("{$wpdb->base_prefix}adrotate", array('bannercode' => $bannercode), array('id' => $advert->id));
			unset($advert, $bannercode);
		}
		adrotate_del_column("{$wpdb->base_prefix}adrotate", 'link');
	}

	// Database: 	56
	// AdRotate:	3.14.2
	if($adrotate_db_version['current'] < 56) {
		adrotate_del_column("{$wpdb->base_prefix}adrotate_tracker", 'useragent');
	}
	...
	if($adrotate_db_version['current'] < 58) {
		$wpdb->query("ALTER TABLE `{$wpdb->base_prefix}adrotate_schedule` CHANGE `dayimpressions` `hourimpressions` int(15) NOT NULL default '0';");
	}
...
function adrotate_optimize_database() {
	...
	if($adrotate_db_timer < ($now - 86400)) {
		dbDelta("OPTIMIZE TABLE `{$wpdb->base_prefix}adrotate`, `{$wpdb->base_prefix}adrotate_groups`, `{$wpdb->base_prefix}adrotate_linkmeta`, `{$wpdb->base_prefix}adrotate_stats`, `{$wpdb->base_prefix}adrotate_stats_archive`, `{$wpdb->base_prefix}adrotate_tracker`, `{$wpdb->base_prefix}adrotate_schedule`;");
		dbDelta("REPAIR TABLE `{$wpdb->base_prefix}adrotate`, `{$wpdb->base_prefix}adrotate_groups`, `{$wpdb->base_prefix}adrotate_linkmeta`, `{$wpdb->base_prefix}adrotate_stats`, `{$wpdb->base_prefix}adrotate_stats_archive`, `{$wpdb->base_prefix}adrotate_tracker`, `{$wpdb->base_prefix}adrotate_schedule`;");
...
function adrotate_cleanup_database() {
	...
	if(isset($_POST['adrotate_db_cleanup_schedules'])) {
		$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_schedule` WHERE `stoptime` < $now;");
	}

	// Delete old stats
	if(isset($_POST['adrotate_db_cleanup_statistics'])) {
		$lastyear = $now - 30758400;
		$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_stats` WHERE `thetime` < $lastyear;");
	}

	// Clean up Tracker data
	$yesterday = $now - 2073600;
	$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_tracker` WHERE `timer` < $yesterday;");

	// Delete empty ads, groups and schedules
	$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate` WHERE `type` = 'empty' OR `type` = 'a_empty';");
	$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_groups` WHERE `name` = '';");
	
	// Clean up meta data
	$ads = $wpdb->get_results("SELECT `id` FROM `{$wpdb->base_prefix}adrotate` ORDER BY `id`;");
	$metas = $wpdb->get_results("SELECT `id`, `ad` FROM `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` != '0' ORDER BY `id`;");
	...
	foreach($result as $key => $value) {
		$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `id` = $key;");
		unset($result[$key]);
	}
	unset($ads, $metas, $adverts, $linkmeta, $result);

	// Clean up stray linkmeta
	$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` = 0 OR `ad` = '';");
...
function adrotate_clean_trackerdata() {
	...
	$wpdb->query("DELETE FROM `{$wpdb->base_prefix}adrotate_tracker` WHERE (`timer` < ".$clicks." AND `stat` = 'c') OR (`timer` < ".$impressions." AND `stat` = 'i') OR `ipaddress`  = 'unknown' OR `ipaddress`  = '';");
}
