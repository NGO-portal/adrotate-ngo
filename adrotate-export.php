...
function adrotate_export_ads($ids, $format) {
	...
	$all_ads = $wpdb->get_results("SELECT * FROM `{$wpdb->base_prefix}adrotate` ORDER BY `id` ASC;", ARRAY_A);

	$ads = array();
	foreach($all_ads as $single) {
		if(in_array($single['id'], $ids)) {
			$starttime = $stoptime = 0;
			$starttime = $wpdb->get_var("SELECT `starttime` FROM `{$wpdb->base_prefix}adrotate_schedule`, `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` = '".$single['id']."' AND `schedule` = `{$wpdb->base_prefix}adrotate_schedule`.`id` ORDER BY `starttime` ASC LIMIT 1;");
			$stoptime = $wpdb->get_var("SELECT `stoptime` FROM `{$wpdb->base_prefix}adrotate_schedule`, `{$wpdb->base_prefix}adrotate_linkmeta` WHERE `ad` = '".$single['id']."' AND  `schedule` = `{$wpdb->base_prefix}adrotate_schedule`.`id` ORDER BY `stoptime` DESC LIMIT 1;");
